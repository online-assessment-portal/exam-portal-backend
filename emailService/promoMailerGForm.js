...
function prepareAndSend(to) {
  return new Promise(async (resolve, reject) => {
    ...
    let htmlBodyCopy = htmlBody;
    htmlBodyCopy = htmlBodyCopy.replace(/\(\(=SentToGreeting=\)\)/g, to);
    const unsubUrl = `https://docs.google.com/forms/d/e/1FAIpQLSc6JHIhqbOprNpEOmyq5B3zA5kFIbWPupKKeiS1fyjLAIU8cQ/formResponse?entry.299090640=${to}&entry.767979255=${mailUID}`;
    htmlBodyCopy = htmlBodyCopy.replace(/\(\(=UnsubLink=\)\)/g, unsubUrl);
    // Set new parameter value for mail Object
    mailObject.to = to;
    mailObject.html = htmlBodyCopy;
